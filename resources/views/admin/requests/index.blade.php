<x-app-layout>
    <x-slot name="title">Pengajuan Buku</x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-muted-foreground mt-1">Kelola permintaan penambahan buku dari siswa.</p>
            </div>
            <div class="flex items-center gap-2 bg-card border border-border rounded-xl px-4 py-2 shadow-sm">
                <div class="w-2 h-2 bg-amber-400 rounded-full animate-pulse"></div>
                <span class="text-sm font-semibold text-muted-foreground">
                    {{ $requests->total() }} Total Pengajuan
                </span>
            </div>
        </div>
        ...
        <div class="bg-card rounded-2xl border border-border shadow-sm p-5">
            <form action="{{ route('admin.requests.index') }}" method="GET" class="flex flex-col lg:flex-row gap-3">
                {{-- Search Input --}}
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                           placeholder="Cari judul, penulis, atau nama pemohon..."
                ...
                {{-- Status Filter --}}
                <div class="sm:w-48" x-data="{ open: false }">
                    <select name="status"
                ...
                <table class="min-w-full divide-y divide-border">
                    <thead>
                        <tr class="bg-muted/80">
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Pemohon</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Detail Buku</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden lg:table-cell">Alasan</th>
                            ...
                            <tr class="hover:bg-muted/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($req->user->name) }}&background=e0e7ff&color=4f46e5&size=36&font-size=0.4&bold=true"
                                             alt="Avatar" class="w-9 h-9 rounded-full flex-shrink-0">
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-foreground truncate">{{ $req->user->name }}</p>
                                            <div class="flex items-center gap-1.5 mt-0.5">
                                                @if($req->user->kelas)
                                                    <span class="text-[10px] font-bold text-primary bg-primary/10 px-1.5 py-0.5 rounded">{{ $req->user->kelas }}</span>
                                                @endif
                                                <span class="text-xs text-muted-foreground">{{ $req->created_at->format('d M Y') }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-muted-foreground mt-0.5">{{ $req->penulis }}</p>
                                        @if($req->penerbit)
                                            <p class="text-xs text-muted-foreground mt-0.5 font-mono">{{ $req->penerbit }}</p>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 hidden lg:table-cell">
                                    <p class="text-sm text-muted-foreground max-w-[200px] truncate">{{ $req->alasan ?? '—' }}</p>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    @if($req->status === 'pending')
                                        <div class="flex items-center justify-end gap-1.5">
                                            <x-confirm-modal
                                                :action="route('admin.requests.update', $req)"
                                                method="PATCH"
                                                title="Setujui Pengajuan?"
                                                message="Setujui pengajuan buku <strong class='text-foreground'>{{ $req->judul_buku }}</strong>?"
                                            ...
                                            <x-confirm-modal
                                                ...
                                                message="Tolak pengajuan buku <strong class='text-foreground'>{{ $req->judul_buku }}</strong>?"
                                ...
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-xs text-muted-foreground mt-1">
                                            ...
                                            @else
                                                Pengajuan buku dari siswa akan muncul di sini.
                                            @endif
                                        </p>
                                        @if($search || $status)
                                            <a href="{{ route('admin.requests.index') }}" class="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:text-primary/80 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                                Reset Pencarian
                                            </a>
                                        @endif
